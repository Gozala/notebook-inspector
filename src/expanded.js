...
export default function inspectExpanded(object /*:Object*/) {
  ...
  a.addEventListener("mouseup", function(event /*:MouseEvent*/) {
  ...
  if (!next.done) {
    ...
    a.addEventListener("mouseup", function(event /*:MouseEvent*/) {
      event.stopPropagation()
      span.insertBefore(value, a)

      for (let i = 0; !(next = fields.next()).done && i < 19; ++i) {
        if (!next.done) {
          span.insertBefore(next.value, a)
        }
      }
...
function* iterateProperty(key, object) {
  ...
  const [name, className] =
    typeof key === "string"
      ? [key, "observablehq--key"]
      : [formatSymbol(key), "observablehq--symbol"]
...
const formatPrototype = (name, object, className) =>
  formatField(
    name,
    Object.getPrototypeOf(object),
    `${className} observablehq--prototype`
  )
...
const formatElement = (index, value, className) =>
  formatField(index, value, className)

const formatGetter = (name, value, className) =>
  formatField(`get ${name}`, value, className)

const formatSetter = (name, value, className) =>
  formatField(`set ${name}`, value, className)
